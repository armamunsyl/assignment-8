...
const Installation = () => {
  ...
  const handleUninstall = (id) => {
    ...
    const uninstalledApp = installed.find((app) => app.id === id);
    setInstalled(updated);
    localStorage.setItem("installedApps", JSON.stringify(updated));
    toast.warning(`${uninstalledApp?.title || "App"} uninstalled`);
  };
    ...
    <div className="bg-[#f9fafb]">
      ...
      <div className="min-h-screen bg-[#f9fafb] px-6 py-8">
        <div className="flex justify-between items-center mb-6">
          <div>
            <p className="text-gray-600 text-sm">{installed.length} Apps Found</p>
          </div>
            ...
            {sortedApps.map((app) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  />
                  <div>
                    <h2 className="font-semibold text-gray-800 text-lg">{app.title}</h2>
